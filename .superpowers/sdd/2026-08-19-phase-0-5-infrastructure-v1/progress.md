...
Task 15: complete — commits `b71bcea` and `e78770f` plus evidence commits; independent re-review spec PASS and quality APPROVED with no findings. Rust contract tests pin exact RuntimeMessage v1 bytes, all 13 generated policies, all 12 ErrorCodes, legal/illegal matrices, golden round trips, every split point, feed limits, EOF/fatal/reset behavior, and atomic batch failure. Rust/C++ decoders use lazy fixed logical frame storage rather than allocator-dependent vector capacity, with deterministic C++ allocation-failure coverage. Debug/Release CTest are 23/23 on macOS arm64.

Task 16: complete — commits `1100d49`, `bd76484`, and `28317c3`; final gate review spec PASS and quality APPROVED with no findings. Real process integration covers Unicode paths, handshake/capabilities/correlation, graceful and abnormal exit, malformed and hostile peers, stderr backpressure, stale generations, concurrent shutdown, actor/runtime cancellation, and concrete-PID no-orphan barriers. Accepted replies remain owned until actual child reap; cancellation transfers ownership through a cancel-safe reap barrier to an independent standard-thread reaper with capped backoff and fail-closed retention. Debug/Release CTest are 24/24 on macOS arm64, the 11-case process gate passed ten consecutive runs, and real Windows execution remains Task 18.

Task 17: complete — commit `bc4bfe7`; independent review spec PASS and quality APPROVED with no findings. The standalone `tests/benchmark` gate dynamically loads the production Mock plugin through the production loader and C ABI, freezes literal sample/byte output and checksum `0x3ecd5190f6f4f725`, validates simulated work, reset/generation, exact success/failure metrics, and the 80-byte RuntimeMessage summary without PCM. The gate has a 20-second CTest timeout; Debug/Release suites are 25/25 on macOS arm64 and focused benchmark runs passed ten consecutive times in each configuration.

Task 18: implementation ready, external acceptance pending — commit `110acc8`; independent review found the workflow implementation spec-conformant and quality APPROVED with no findings. SHA-pinned, cache-independent `macos-15` arm64 and `windows-latest` x64/MSVC jobs cover contracts, pnpm, Cargo, Debug/Release CTest, Tauri, Windows export/DLL-load/process-reap gates, and portable resource layout. Local macOS-equivalent and workflow/fixture validation pass. The plan item remains open because this repository has no Git remote and no real Windows runner URL/log exists; a successful remote Windows job is a hard acceptance gate.

Task 19: complete — commits `9601e6e` and `47abd5c`; final gate review spec PASS and quality APPROVED with no findings. DEVELOPMENT and ARCHITECTURE now document reproducible toolchains, commands, the implemented Tauri-to-Mock control chain, process and trust boundaries, configuration/capability/logging contracts, repository ownership, exclusions, and the pending Windows gate. ADR-000/001/002 contain complete decision history aligned with implementation; ADR-003 remains reserved and absent. Authority-bound documentation tests validate ordered architecture edges, DTO fields, versions, commands, links/anchors, Windows status, and adversarial drift. Debug/Release CTest are 25/25 on macOS arm64; documentation contracts are 8/8.

Task 20: local acceptance approved, overall acceptance pending — commits `cd1475e`, `0ae7425`, and `fb3de90`; final independent gate found implementation/report spec PASS, quality APPROVED, and no findings. The PENDING acceptance report maps all 13 requirements, records six bundle artifacts and authoritative versions, and is bound to a deterministic live evidence manifest. Fail-closed inspection verifies exact bundle contents, arm64 Mach-O types, dependencies, imported/global symbols, dependency closure, excluded model/audio/archive assets, and bounded application start/exit without orphan processes. The 21-case adversarial gate passes and the live manifest is byte-stable at SHA-256 `4a9413327709c494663d65a3bd5b276b40a095f6d86d94734b0bdfb726975e77`. The plan item remains open together with Task 18 because a successful real Windows x64 CI URL/log is the sole remaining hard gate.
